...
function omega_entry_published_shortcode( $attr ) {
	...
	$published = '<time class="published" title="' . get_the_time( esc_attr__( 'l, F jS, Y, g:i a', 'hybrid-core' ) ) . '" itemprop="datePublished">' . get_the_time( $attr['format'] ) . '</time>';
...
function omega_entry_author_shortcode( $attr ) {
	...
	$author = '<span class="author vcard" itemptype="http://schema.org/Person" itemscope="itemscope" itemprop="author"><a class="url fn n" rel="author" itemprop="url" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '" title="' . esc_attr( get_the_author_meta( 'display_name' ) ) . '"><span itemprop="name" class="entry-author-name">' . get_the_author_meta( 'display_name' ) . '</span></a></span>';
